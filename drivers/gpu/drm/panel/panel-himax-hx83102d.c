...
static __maybe_unused int hx83102d_generic_read(struct hx83102d *ctx,
				 u8 cmd, void *data, size_t len)
{
	...
	if (ret < 0)
		dev_err(ctx->dev, "error %d read generic seq(%#x)\n", ret, cmd);
	return ret;
}

static __maybe_unused int hx83102d_dcs_read(struct hx83102d *ctx,
				 u8 cmd, void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	int ret;
	...
	if (ret < 0)
		dev_err(ctx->dev, "error %d read dcs seq(%#x)\n", ret, cmd);
	return ret;
}

static __maybe_unused int hx83102d_dcs_dump(struct hx83102d *ctx,
					 u8 cmd, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	int ret;
