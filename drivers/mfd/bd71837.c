						...
						int *chip_id)
{
	...
	board_info->enable_mipi_susres = of_property_read_bool(np, "shes,enable_mipi_susres");
	dev_info(&client->dev, "enable_mipi_susres %d\n",board_info->enable_mipi_susres);
...
static int bd71837_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bd71837 *bd71837 = i2c_get_clientdata(client);

	if (bd71837->of_plat_data->enable_mipi_susres) {
		bd71837_reg_write(bd71837, BD71837_REG_LDO6_VOLT, 0x83);//1.2v
		usleep_range(10000, 11000);
		bd71837_reg_write(bd71837, BD71837_REG_LDO4_VOLT, 0x80);//0.9v
	}

	return 0;
}

static int bd71837_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bd71837 *bd71837 = i2c_get_clientdata(client);

	if (bd71837->of_plat_data->enable_mipi_susres) {
		bd71837_reg_write(bd71837, BD71837_REG_LDO4_VOLT, 0x40);//0.9v
		usleep_range(10000, 11000);
		bd71837_reg_write(bd71837, BD71837_REG_LDO6_VOLT, 0x43);//1.2v
	}

	return 0;
}

static SIMPLE_DEV_PM_OPS(bd71837_pm_ops, bd71837_suspend, bd71837_resume);
