...
#if defined(HX_EN_DYNAMIC_NAME) || defined(HX_ZERO_FLASH)
...
void himax_extcon_typec_get_state(struct himax_ts_data *ts)
{
	int state;

	if (IS_ERR_OR_NULL(ts->extcon_dev)) {
		W("%s: May failed get USB TypeC extcon device \n", __func__);
		return;
	}

	state = extcon_get_state(ts->extcon_dev, EXTCON_USB);
	if (state < 0) {
		E("%s: Can not get extcon USB TypeC state  state=%d\n",
			__func__, state);
		return;
	}
	USB_detect_flag = (bool)state;

}

static int himax_extcon_typec_event_cb(struct notifier_block *nb,
	unsigned long event, void *ptr)
{
	USB_detect_flag = (bool)event;
	D("%s: Call Back TYPEC event=%ld USB_detect_flag=%s \n",
		__func__, event, USB_detect_flag ? "true" : "false");

	return NOTIFY_OK;
}

void himax_extcon_typec_register(struct himax_ts_data *ts)
{
	int ret = 0;

	if (!IS_ERR_OR_NULL(ts->extcon_dev)) {
		W("%s: USB TypeC extcon device already registered\n", __func__);
		return;
	}

	if (!of_property_read_bool(ts->dev->of_node, "extcon")) {
		E("%s: extcon device tree read error\n", __func__);
		return;
	}

	ts->extcon_dev = extcon_get_edev_by_phandle(ts->dev, EXTCON_OF_TYPEC);
	if (IS_ERR_OR_NULL(ts->extcon_dev)) {
		W("%s: Can not get USB TypeC extcon device ret=%d\n",
			__func__, ret);
		return;
	}

	ts->notifier_block.notifier_call = himax_extcon_typec_event_cb;
	ret = extcon_register_notifier(ts->extcon_dev, EXTCON_USB,
		&(ts->notifier_block));
	if (ret)
		E("%s: Failed to register extcon call back ret=%d\n",
			__func__, ret);
}

void himax_extcon_typec_unregister(struct himax_ts_data *ts)
{
	int ret = 0;
	...
	ret = extcon_unregister_notifier(ts->extcon_dev, EXTCON_USB,
		&(ts->notifier_block)
	);
	if (ret != 0) {
		E("%s: Failed to unregister EXTCON_USB ret=%d\n",
			__func__, ret);
		return;
	}
	ts->extcon_dev = NULL;
...
void himax_cable_detect_func(bool force_renew)
{
	...
	if (IS_ERR_OR_NULL(ts->extcon_dev)) {
		himax_extcon_typec_register(ts);
		himax_extcon_typec_get_state(ts);
	}
...
static int himax_err_ctrl(struct himax_ts_data *ts, uint8_t *buf, int ts_path, int ts_status)
{
	...
	if (HX_HW_RESET_ACTIVATE) {
		...
		HX_HW_RESET_ACTIVATE = 0;
		D("[HX_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n", __func__);
		ts_status = HX_RST_OK;
		goto END_FUNCTION;
	}
#endif
...
int himax_chip_common_init(void)
{
	...
	if (g_core_chip_dt[i].fp_chip_detect != NULL) {
			if (g_core_chip_dt[i].fp_chip_detect() == true) {
				D("%s: chip detect found! list_num=%d \n", __func__, i);
				goto found_hx_chip;
			}	else {
...
FW_force_upgrade:
	...
	queue_delayed_work(ts->himax_0f_update_wq, &ts->work_0f_update, msecs_to_jiffies(10000));
#endif
...
#endif
#if defined(HX_USB_DETECT_GLOBAL)
	himax_extcon_typec_unregister(ts);
#endif
