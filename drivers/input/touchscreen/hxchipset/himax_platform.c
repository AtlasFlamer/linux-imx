					...
					struct himax_i2c_platform_data *pdata)
{
	...
	private_ts->cs_gpio = pdata->gpio_cs;

	if (!gpio_is_valid(pdata->gpio_cs)) {
		I(" DT:gpio_cs value is not valid\n");
	}

	D(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d gpio_cs=%d\n", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_3v3_en, pdata->gpio_cs);

	if (of_property_read_u32(dt, "report_type", &data) == 0) {
		pdata->protocol_type = data;
		D(" DT:protocol_type=%d\n", pdata->protocol_type);
	}

#ifdef HX_ZERO_FLASH
	rc = of_property_read_string_index(dt, "fw_image", 0, (const char **)&i_CTPM_firmware_name);
	if (rc != 0)
		E(" DT:firmware_name read error rc =%d\n", rc);
	else
		I(" DT:firmware_name %s\n", i_CTPM_firmware_name);
#endif
...
static int himax_spi_read(uint8_t *command, uint8_t command_len, uint8_t *data, uint32_t length, uint8_t toRetry)
{
	...
	exc_rme_tp_lock();
#endif /* LOCAL_CONFIG_EXC_RME_TP */
	gpio_direction_output(private_ts->cs_gpio,0);
	...
	gpio_direction_output(private_ts->cs_gpio,1);
#ifdef LOCAL_CONFIG_EXC_RME_TP
	exc_rme_tp_unlock();
#endif /* LOCAL_CONFIG_EXC_RME_TP */
...
int himax_common_suspend_export(void)
{
	struct himax_ts_data *ts = private_ts;

	D("%s: enter\n", __func__);
	if (ts != NULL && ts->dev != NULL)
		himax_chip_common_suspend(ts);

	I("[HXTP] touch panel disabled\n");
	return 0;
}
EXPORT_SYMBOL(himax_common_suspend_export);

int himax_common_resume_export(void)
{
	struct himax_ts_data *ts = private_ts;
	...
	I("[HXTP] touch panel enabled\n");

	return 0;
}
EXPORT_SYMBOL(himax_common_resume_export);
...
int himax_common_remove_export(void)
{
	struct himax_ts_data *ts = private_ts;

	I("%s: enter\n", __func__);
	if (ts != NULL && ts->dev != NULL && ts->spi != NULL)
		himax_chip_common_remove(ts->spi);

	pr_info("[HXTP] touch panel removed\n");

	return 0;
}
EXPORT_SYMBOL(himax_common_remove_export);
