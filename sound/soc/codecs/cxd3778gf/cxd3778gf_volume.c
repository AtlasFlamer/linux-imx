...
int cxd3778gf_set_output_device_mute(struct cxd3778gf_status * now, int mute, int pwm_wait)
{
	...
	if (lout_vol_mute && now->output_device == OUTPUT_DEVICE_LINE &&
		(board_set_flag & HW_MUTE_CONSTANT_30MS_FLAG)) {
	...
	if (now->board_type == TYPE_A || (now->board_type == TYPE_Z && now->headphone_amp!=HEADPHONE_AMP_SMASTER_BTL)){
		if(hw_mute){
			...
			if (!(board_set_flag & HW_MUTE_CONSTANT_30MS_FLAG)) {
				if (now->playback_latency == PLAYBACK_LATENCY_NORMAL)
					msleep(130);
				else
					msleep(80);
			}
...
//		}
	...
	if (lout_vol_mute) {
		 ...
		 * (CODEC_PLAYVOL) and should set temporarily faster timeout value
		 * for avoiding discontinuity.
		 */
		if (now->output_device == OUTPUT_DEVICE_LINE &&
			(board_set_flag & HW_MUTE_CONSTANT_30MS_FLAG)) {
			msleep(30);
			cxd3778gf_register_write(CXD3778GF_LINEOUT_VOL, 0x00);
			cxd3778gf_register_write(CXD3778GF_CODEC_SR_PLAY3, timeout_tmp);
		} else
			cxd3778gf_register_write(CXD3778GF_LINEOUT_VOL, 0x00);
	}
