...
static int fsl_micfil_dai_probe(struct snd_soc_dai *cpu_dai)
{
	...
	/* set default qsel from device tree */
	ret = regmap_update_bits(micfil->regmap, REG_MICFIL_CTRL2,
				 MICFIL_CTRL2_QSEL_MASK, micfil->quality);
	if (ret) {
	...
	/* set default gain from device tree */
	remapped_value = (micfil->channel_initial_gain - 8) & 0xF;
	for (i = 0; i < 8; i++) {
		regmap_update_bits(micfil->regmap, REG_MICFIL_OUT_CTRL, 0x0F << (i * 4), remapped_value << (i * 4));
		micfil->channel_gain[i] = micfil->channel_initial_gain;
	}
...
static int fsl_micfil_probe(struct platform_device *pdev)
{
	...
	micfil->quality = qsel << MICFIL_CTRL2_QSEL_SHIFT;

	ret = of_property_read_u32(np,
				"initial_gain",
				&micfil->channel_initial_gain);
	 /* Set max_gain */
	if (ret)
		micfil->channel_initial_gain = 0xF;
